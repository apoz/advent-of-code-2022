...
type StatusQueue struct {
	statuses []QueueElement
}

func bestCaseScenario(initialGemAmount int, robotNum int, time int) int {
	return initialGemAmount + robotNum*(time+1) + time*(time+1)/2
}

func (s *StatusQueue) enqueue(qElem QueueElement) {
	s.statuses = append(s.statuses, qElem)
}

func (s *StatusQueue) dequeue() QueueElement {
	if len(s.statuses) > 0 {
		elem := s.statuses[0]
...
func initialQueueElement() QueueElement {
	gI := GemInventory{
		ore:      0,
		clay:     0,
		obsidian: 0,
...
func search(bp Blueprint, time int) int {
	...
	qElems := make([]QueueElement, 0)
	q := &StatusQueue{
		statuses: qElems,
	}
	q.enqueue(initialQueueElement())
	for q.isEmpty() == false {
		qLen := len(*&q.statuses)
		if (qLen % 1000000) == 0 {
		...
		if time == 0 {
			...
			if newGems.geode > best {
				best = newGems.geode
				fmt.Printf("New max for geodes best: %d\n", best)
			}
		...
		if bestCaseScenario(newGems.geode, status.robs.geode, time) < best {
			continue
		}
		...
		if bestCaseScenario(newGems.obsidian, status.robs.obsidian, time) < bp.robotCost["geode"].obsidian {
			best = max(best, newGems.geode+status.robs.geode*time)
			continue
		}

		// Likewise for ore.
		if bestCaseScenario(newGems.ore, status.robs.ore, time) < bp.robotCost["geode"].ore {
			best = max(best, newGems.geode+status.robs.geode*time)
			continue
		}
		...
		canBuild := make([]string, 0)
		// Have enough material for geode robot? build it
		if canBuildGeodeRobot(status.gems, bp.robotCost["geode"]) && !stringInSlice("geode", didNotBuild) {
			canBuild = append(canBuild, "geode")
			...
			qE := QueueElement{
				status:      nS,
				didNotBuild: make([]string, 0),
			}
			q.enqueue(qE)
		}

		// Have enough material for obsidian robot? build it
		if canBuildObsidianRobot(status.gems, bp.robotCost["obsidian"]) && !stringInSlice("obsidian", didNotBuild) {
			if status.robs.obsidian < max_obs_needed {
				// fmt.Printf("Build obsidian robot minute=%d\n", 24-time+1)
				canBuild = append(canBuild, "obsidian")
				nS := newStatus(time, newGems, status.robs)
				// fmt.Printf("Status before %+v\n", nS)
		...
		if canBuildOreRobot(status.gems, bp.robotCost["ore"]) && !stringInSlice("ore", didNotBuild) {
			if status.robs.ore < max_ore_needed {
				// fmt.Printf("Build ore robot minute=%d\n", 24-time+1)
				canBuild = append(canBuild, "ore")
				nS := newStatus(time, newGems, status.robs)
				// fmt.Printf("Status before %+v\n", nS)
		...
		if canBuildClayRobot(status.gems, bp.robotCost["clay"]) && !stringInSlice("clay", didNotBuild) {
			if status.robs.clay < max_clay_needed {
				// fmt.Printf("Build clay robot minute=%d\n", 24-time+1)
				canBuild = append(canBuild, "clay")
				nS := newStatus(time, newGems, status.robs)
				// fmt.Printf("Status before %+v\n", nS)
...
func main() {
	...
	for i, line := range inputLines {
		...
		resp := search(bp, 32)
		fmt.Printf("Response for bp %d is %d\n", bp.id, resp)
	}
