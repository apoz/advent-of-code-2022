package main

import (
	"bufio"
	"fmt"
	"os"
	"regexp"
	"strconv"
	"strings"
)

type Problem struct {
	board     [][]string
	posDepth  int
	posWidth  int
	direction string
}

func readStdin() []string {
	var lines []string = []string{}
	s := bufio.NewScanner(os.Stdin)
	for s.Scan() {
		lines = append(lines, s.Text())
	}
	return lines
}

func getBoardDimensions(lines []string) (int, int) {
	width := 0
	depth := 0
	for count, line := range lines {
		w := len(strings.Split(line, ""))
		if w > width {
			width = w
		}
		depth = count
	}
	return depth + 1, width
}

func getBoard(lines []string) [][]string {
	_, width := getBoardDimensions(lines)
	board := make([][]string, 0)
	for _, line := range lines {
		line = strings.TrimSuffix(line, "\n")
		elems := strings.Split(line, "")
		if len(elems) < width {
			for i := len(elems); i < width; i++ {
				elems = append(elems, " ")
			}
		}
		board = append(board, elems)
	}
	return board
}

func (p *Problem) printBoard(board [][]string) {
	for count, line := range (*p).board {
		fmt.Printf(" [%d]  ", count)
		for i, c := range line {
			if p.posDepth == count && p.posWidth == i {
				fmt.Print(" X")
			} else {
				fmt.Printf(" %s", c)
			}

		}
		fmt.Printf("\n")
	}
}

func (p *Problem) getStartPosition() (int, int) {
	var width int = 0
	for w, c := range (*p).board[0] {
		if c == "." {
			width = w
			break
		}
	}
	return 0, width
}

func (p *Problem) switch2RightTo4() (int, int, string) {
	// depth examples 2 49 > 4 100
	//                2 0 > 4 149
	newDepth := 149 - ((*p).posDepth)
	newWidth := 99
	newDir := "L"
	return newDepth, newWidth, newDir
}

func (p *Problem) switch3RightTo2() (int, int, string) {
	//  depth examples 3 depth 50 > 2 width 100
	//                 3 depth 99 > 2 width 149
	newDepth := 49
	newWidth := (*p).posWidth + 50
	newDir := "T"
	return newDepth, newWidth, newDir
}

func (p *Problem) switch4RightTo2() (int, int, string) {
	// examples 4 depth 100 > 2 depth 49
	//          4 d 149 > 2 d 0
	newDepth := 149 - (*p).posDepth
	newWidth := 149
	newDir := "L"
	return newDepth, newWidth, newDir
}

func (p *Problem) switch6RightTo4() (int, int, string) {
	// examples 6 d 150 > 4 w 50
	//          6 d 199 > 4 w 99
	newDepth := 149
	newWidth := (*p).posDepth - 100
	newDir := "T"
	return newDepth, newWidth, newDir
}

func (p *Problem) switch5LeftTo1() (int, int, string) {
	// examples 5 d 100 > 1 d 49
	//          5 d 149 > 1 d 0
	newDepth := 149 - (*p).posDepth
	newWidth := 50
	newDir := "R"
	return newDepth, newWidth, newDir
}

func (p *Problem) switch6LeftTo1() (int, int, string) {
	// examples 6 d 150 > 1 w 50
	//          6 d 199 > 1 w 99

	newDepth := 0
	newWidth := (*p).posDepth - 100
	newDir := "B"
	return newDepth, newWidth, newDir
}

func (p *Problem) switch1LeftTo5() (int, int, string) {
	// examples 1 d 0 > 5 d 149
	//          1 d 49 > 5 d 100

	newDepth := 149 - (*p).posDepth
	newWidth := 0
	newDir := "R"
	return newDepth, newWidth, newDir
}

func (p *Problem) switch3LeftTo5() (int, int, string) {
	// examples 3 d 50 > 5 w 0
	//          3 d 99 > 5 w 49

	newDepth := 100
	newWidth := (*p).posDepth - 50
	newDir := "B"
	return newDepth, newWidth, newDir
}

func (p *Problem) switch1TopTo6() (int, int, string) {
	// examples 1 w 50 > 6 d 150
	//          1 w 99 > 6 d 199

	newDepth := 100 + (*p).posWidth
	newWidth := 0
	newDir := "R"
	return newDepth, newWidth, newDir
}

func (p *Problem) switch2TopTo6() (int, int, string) {
	// examples 2 w 100 > 6 w 0
	//          2 w 149 > 6 w 49

	newDepth := 199
	newWidth := (*p).posWidth - 100
	newDir := "T"
	return newDepth, newWidth, newDir
}

func (p *Problem) switch5TopTo3() (int, int, string) {
	// examples 5 w 0 > 3 d 50
	//          5 w 49 > 3 d 99

	newDepth := 50 + (*p).posWidth
	newWidth := 50
	newDir := "R"
	return newDepth, newWidth, newDir
}

func (p *Problem) switch6BottomTo2() (int, int, string) {
	// examples 6 w 0 > 2 w 100
	//          6 w 49 > 2 w 149

	newDepth := 0
	newWidth := 100 + (*p).posWidth
	newDir := "B"
	return newDepth, newWidth, newDir
}

func (p *Problem) switch4BottomTo6() (int, int, string) {
	// examples 4 w 50 > 6 w 150
	//          4 w 99 > 6 w 199

	newDepth := 100 + (*p).posWidth
	newWidth := 49
	newDir := "L"
	return newDepth, newWidth, newDir
}

func (p *Problem) switch2BottomTo3() (int, int, string) {
	// examples 2 w 100 > 3 d 50
	//          2 w 149 > 3 d 99

	newDepth := (*p).posWidth - 50
	newWidth := 99
	newDir := "L"
	return newDepth, newWidth, newDir
}

func (p *Problem) getFirstPositionAndValueInRow(depth int) (int, string) {
	i := 0
	for {
		if (*p).board[depth][i] == " " {
			i++
			continue
		} else {
			return i, (*p).board[depth][i]
		}
	}
}

func (p *Problem) getLastPositionAndValueInRow(depth int) (int, string) {
	i := len((*p).board[depth]) - 1
	for {
		if (*p).board[depth][i] == " " {
			i--
			continue
		} else {
			return i, (*p).board[depth][i]
		}

	}
}

func (p *Problem) getFirstPositionAndValueInColumn(width int) (int, string) {
	i := 0
	for {
		if (*p).board[i][width] == " " {
			i++
			continue
		} else {
			return i, (*p).board[i][width]
		}
	}
}

func (p *Problem) getLastPositionAndValueInColumn(width int) (int, string) {
	i := len((*p).board) - 1
	for {
		if (*p).board[i][width] == " " {
			i--
			continue
		} else {
			return i, (*p).board[i][width]
		}
	}
}

func (p *Problem) updateDirection(dir string) {
	if (*p).direction == "R" && dir == "R" {
		(*p).direction = "B"
	} else if (*p).direction == "B" && dir == "R" {
		(*p).direction = "L"
	} else if (*p).direction == "L" && dir == "R" {
		(*p).direction = "T"
	} else if (*p).direction == "T" && dir == "R" {
		(*p).direction = "R"
	} else if (*p).direction == "R" && dir == "L" {
		(*p).direction = "T"
	} else if (*p).direction == "T" && dir == "L" {
		(*p).direction = "L"
	} else if (*p).direction == "L" && dir == "L" {
		(*p).direction = "B"
	} else if (*p).direction == "B" && dir == "L" {
		(*p).direction = "R"
	} else {
		fmt.Println("Unexpected direction move!")
	}
}

func (p *Problem) makeMoveRight() {
	if (*p).posWidth == len((*p).board[(*p).posDepth])-1 {
		// for sure it´s 2´s right
		if (*p).board[(*p).posDepth][(*p).posWidth] == "." {
			newDepth, newWidth, newDir := p.switch2RightTo4()
			// pos, val := p.getFirstPositionAndValueInRow(p.posDepth)
			if (*p).board[newDepth][newWidth] == "." {
				p.posDepth = newDepth
				p.posWidth = newWidth
				p.direction = newDir
			} else {
				return
			}
		}
	} else if (*p).board[(*p).posDepth][(*p).posWidth+1] == "." {
		(*p).posWidth = (*p).posWidth + 1
	} else if (*p).board[(*p).posDepth][(*p).posWidth+1] == "#" {
		return
	} else if (*p).board[(*p).posDepth][(*p).posWidth+1] == " " {
		if (*p).posDepth >= 50 && (*p).posDepth < 100 {
			// we´re going 3 right
			newDepth, newWidth, newDir := p.switch3RightTo2()
			if (*p).board[newDepth][newWidth] == "." {
				p.posDepth = newDepth
				p.posWidth = newWidth
				p.direction = newDir
			} else {
				return
			}
		} else if (*p).posDepth >= 100 && (*p).posDepth < 150 {
			// 4 right
			newDepth, newWidth, newDir := p.switch4RightTo2()
			if (*p).board[newDepth][newWidth] == "." {
				p.posDepth = newDepth
				p.posWidth = newWidth
				p.direction = newDir
			} else {
				return
			}
		} else if (*p).posDepth >= 150 && (*p).posDepth < 200 {
			// 6 right
			newDepth, newWidth, newDir := p.switch6RightTo4()
			if (*p).board[newDepth][newWidth] == "." {
				p.posDepth = newDepth
				p.posWidth = newWidth
				p.direction = newDir
			} else {
				return
			}
		}
	}
}

func (p *Problem) makeMoveLeft() {
	if (*p).posWidth == 0 {
		if (*p).board[(*p).posDepth][(*p).posWidth] == "." {
			if (*p).posDepth >= 100 && (*p).posDepth < 150 {
				// we´re going 5 left
				newDepth, newWidth, newDir := p.switch5LeftTo1()
				if (*p).board[newDepth][newWidth] == "." {
					p.posDepth = newDepth
					p.posWidth = newWidth
					p.direction = newDir
				} else {
					return
				}
			} else if (*p).posDepth >= 150 && (*p).posDepth < 200 {
				// we´re going 6 left
				newDepth, newWidth, newDir := p.switch6LeftTo1()
				if (*p).board[newDepth][newWidth] == "." {
					p.posDepth = newDepth
					p.posWidth = newWidth
					p.direction = newDir
				} else {
					return
				}
			}
		}
	} else if (*p).board[(*p).posDepth][(*p).posWidth-1] == "." {
		(*p).posWidth = (*p).posWidth - 1
	} else if (*p).board[(*p).posDepth][(*p).posWidth-1] == "#" {
		return
	} else if (*p).board[(*p).posDepth][(*p).posWidth-1] == " " {
		if (*p).posDepth >= 0 && (*p).posDepth < 50 {
			// we´re going 1 left
			newDepth, newWidth, newDir := p.switch1LeftTo5()
			if (*p).board[newDepth][newWidth] == "." {
				p.posDepth = newDepth
				p.posWidth = newWidth
				p.direction = newDir
			} else {
				return
			}
		} else if (*p).posDepth >= 50 && (*p).posDepth < 99 {
			// we´re going 3 left
			newDepth, newWidth, newDir := p.switch3LeftTo5()
			if (*p).board[newDepth][newWidth] == "." {
				p.posDepth = newDepth
				p.posWidth = newWidth
				p.direction = newDir
			} else {
				return
			}
		}
	}
}

func (p *Problem) makeMoveTop() {
	if (*p).posDepth == 0 {
		if (*p).board[(*p).posDepth][(*p).posWidth] == "." {
			if (*p).posWidth >= 50 && (*p).posWidth < 100 {
				// we´re going 1 top
				newDepth, newWidth, newDir := p.switch1TopTo6()
				if (*p).board[newDepth][newWidth] == "." {
					p.posDepth = newDepth
					p.posWidth = newWidth
					p.direction = newDir
				} else {
					return
				}
			} else if (*p).posWidth >= 100 && (*p).posWidth < 150 {
				// we´re going 2 top
				newDepth, newWidth, newDir := p.switch2TopTo6()
				if (*p).board[newDepth][newWidth] == "." {
					p.posDepth = newDepth
					p.posWidth = newWidth
					p.direction = newDir
				} else {
					return
				}
			}
		}
	} else if (*p).board[(*p).posDepth-1][(*p).posWidth] == "." {
		(*p).posDepth = (*p).posDepth - 1
	} else if (*p).board[(*p).posDepth-1][(*p).posWidth] == "#" {
		return
	} else if (*p).board[(*p).posDepth-1][(*p).posWidth] == " " {
		if (*p).posWidth >= 0 && (*p).posWidth < 50 {
			// we´re going 5 top
			newDepth, newWidth, newDir := p.switch5TopTo3()
			if (*p).board[newDepth][newWidth] == "." {
				p.posDepth = newDepth
				p.posWidth = newWidth
				p.direction = newDir
			} else {
				return
			}
		}
	}
}

func (p *Problem) makeMoveBottom() {

	if (*p).posDepth == len((*p).board)-1 {
		if (*p).board[(*p).posDepth][(*p).posWidth] == "." {
			if (*p).posWidth >= 0 && (*p).posWidth < 50 {
				// we´re going 6 bottom
				newDepth, newWidth, newDir := p.switch6BottomTo2()
				if (*p).board[newDepth][newWidth] == "." {
					p.posDepth = newDepth
					p.posWidth = newWidth
					p.direction = newDir
				} else {
					return
				}
			}
		}
	} else if (*p).board[(*p).posDepth+1][(*p).posWidth] == "." {
		(*p).posDepth = (*p).posDepth + 1
	} else if (*p).board[(*p).posDepth+1][(*p).posWidth] == "#" {
		return
	} else if (*p).board[(*p).posDepth+1][(*p).posWidth] == " " {
		if (*p).posWidth >= 50 && (*p).posWidth < 100 {
			// we´re going 4 bottom
			newDepth, newWidth, newDir := p.switch4BottomTo6()
			if (*p).board[newDepth][newWidth] == "." {
				p.posDepth = newDepth
				p.posWidth = newWidth
				p.direction = newDir
			} else {
				return
			}
		} else if (*p).posWidth >= 100 && (*p).posWidth < 150 {
			// we´re going 2 bottom
			newDepth, newWidth, newDir := p.switch2BottomTo3()
			if (*p).board[newDepth][newWidth] == "." {
				p.posDepth = newDepth
				p.posWidth = newWidth
				p.direction = newDir
			} else {
				return
			}
		}
	}
}

func (p *Problem) printResult() {
	dirs := map[string]int{
		"R": 0,
		"B": 1,
		"L": 2,
		"T": 3,
	}
	response := 1000*((*p).posDepth+1) + 4*((*p).posWidth+1) + dirs[(*p).direction]
	fmt.Printf("Response is %d\n", response)
}

func (p *Problem) makeMove(steps int) {
	for i := 0; i < steps; i++ {
		if p.direction == "R" {
			p.makeMoveRight()
		} else if p.direction == "L" {
			p.makeMoveLeft()
		} else if p.direction == "T" {
			p.makeMoveTop()
		} else if p.direction == "B" {
			p.makeMoveBottom()
		}
	}
}

func getMovs(line string) ([]int, []string) {
	reg, _ := regexp.Compile(`\d+`)
	reg2, _ := regexp.Compile(`\D+`)
	numbers := reg.FindAllString(line, -1)
	changes := reg2.FindAllString(line, -1)
	integers := make([]int, 0)
	for _, s := range numbers {
		n, _ := strconv.Atoi(s)
		integers = append(integers, n)
	}
	return integers, changes
}

func main() {

	// var valves map[string]Valve = make(map[string]Valve)
	// var movements string = "10R5L5R10L4R5L5"
	var movements string = ""
	inputLines := readStdin()
	_ = movements
	board := getBoard(inputLines)
	p := &Problem{
		board:     board,
		posDepth:  0,
		posWidth:  0,
		direction: "R",
	}
	depth, width := p.getStartPosition()
	p.posDepth = depth
	p.posWidth = width
	p.printBoard(board)
	fmt.Printf("Board start pos depth %d width %d \n", p.posDepth, p.posWidth)
	numbers, directions := getMovs(movements)
	fmt.Printf("Numbers %+v  Directions %+v\n", numbers, directions)
	for i, steps := range numbers {
		p.makeMove(steps)
		fmt.Printf("After move %d steps %s\n", steps, p.direction)
		p.printBoard(p.board)
		if i < len(directions) {
			p.updateDirection(directions[i])
		}
		fmt.Printf("Updated directions to %s\n", p.direction)
	}
	p.printResult()
}
