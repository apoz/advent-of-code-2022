...
func (p *Problem) sandBlockFall() bool {
	...
	for canMove == true {
		...
		if currentDepth == len(p.matrix)-1 || currentDist == len(p.matrix[0])-1 || currentDist == 0 {
			canMove = false
			fallDown = true
		} else if p.matrix[currentDepth][currentDist] == "+" || p.matrix[currentDepth+1][currentDist] == "." {
			...
			if currentDist == 0 || currentDepth == 0 || currentDist == len(p.matrix[0]) || currentDepth == len(p.matrix) {
				canMove = false
				fallDown = true
			}
...
func (p *Problem) getMatrix(lines []string) [][]string {
	...
	matrix := p.getBaseMatrix(distLimit, depthLimit)
	p.matrix = matrix
	p.printMatrix()
...
func (p *Problem) processDatapoints() {
	for i := 0; i < len(p.dataPoints); i++ {
		for j := 0; j < (len(p.dataPoints[i]) - 1); j++ {
			fmt.Printf("orig datapoint %+v\n", p.dataPoints[i][j])
			fmt.Printf("dest datapoint %+v\n", p.dataPoints[i][j+1])
			dps := p.interpolateDatapoints(p.dataPoints[i][j], p.dataPoints[i][j+1])
			fmt.Printf("Interpolated data %+v\n", dps)
			for _, dp := range dps {
				fmt.Println("about to call setMatrixDatapoint %+v\n", dp)
...
func (p *Problem) setMatrixDatapoint(dist int, depth int, value string) {
	fmt.Printf("In set Matrix data point x %d, y %d value %s \n", dist, depth, value)
	if dist < p.distBase {
