...
func getRequiredPlayerChoice(elfColumn string, playerColumn string) string {
	// elf Column A for Rock, B for Paper, and C for Scissors
	// player choice X means you need to lose, Y draw, and Z you win.
	switch playerColumn {
	case "X": //lose
		switch elfColumn {
		case "A": // elf ROCK
	...
	case "Y": // draw
		return getRPSValue(elfColumn) //same choice as elf
	case "Z": //win
		switch elfColumn {
		case "A": // elf ROCK
			return "PAPER"
...
func main() {
	...
	for _, line := range inputLines {
		...
		playerChoice := getRequiredPlayerChoice(choices[0], choices[1])
