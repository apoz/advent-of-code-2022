...
func minAllDirsBiggerThan(rootDir *directory, sizeLimit int) int {
	var minSize int = 999999999
	getAllDirs(rootDir)
	for _, dir := range allDirs {
		dirSize := dir.totalSize()
		if dirSize > sizeLimit {
			println(dir.Path, "/", dir.Name, " is bigger than ", sizeLimit, " (", dirSize, ")")
			if dirSize < minSize {
				minSize = dirSize
			}
		}
...
func printAllDirsSize(rootDir *directory) {
	getAllDirs(rootDir)
	for _, dir := range allDirs {
		dirSize := dir.totalSize()
		println(dir.Path+dir.Name, " is ", dirSize)
	}
}
...
func main() {
	...
	minSize := minAllDirsBiggerThan(rootDir, 10822529)
	println("Response ", minSize)
