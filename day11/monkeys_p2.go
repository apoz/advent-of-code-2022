...
func (m *Monkey) appendItemRound(item uint64) {
...
func (m *Monkey) calculateOperation(item uint64) uint64 {
...
func main() {
	...
	for _, mon := range Monkeys {
		superm = superm * (*mon).testDivisible
	}
	//println("SUPERMODULO ", superm)
	// Weird thing I had to check
	// In order NOT to modify the modulo operation for a number
	// we can mod the number to the least common multiple of all mod divisors
	// not to modify the results of ANY of the monkeys divisors

	for _, mon := range Monkeys {
		mon.supermodulo = superm
	}
	...
	for rounds := 0; rounds < 10000; rounds++ {
		...
		// 		fmt.Printf("Monkey %d: %d\n", i, mon.inspectCounter)
		// 	}
		// 	println("==============================")
		// }
	}

	resMonkeys := make([]uint64, 0)
	for i, mon := range Monkeys {
		fmt.Println("Monkey ", i, " ->", mon.inspectCounter)
		resMonkeys = append(resMonkeys, mon.inspectCounter)
	}
	//sort.Sort(resMonkeys)
	sort.Slice(resMonkeys, func(i, j int) bool { return resMonkeys[i] < resMonkeys[j] })
