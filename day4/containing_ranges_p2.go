...
func isSolaped(ass1 assignment, ass2 assignment) bool {
	if isContained(ass1, ass2) {
		return true
	}
	if (ass1.lower <= ass2.lower) &&
		(ass2.lower <= ass1.higher) {
		return true
	}
	if (ass2.lower <= ass1.lower) &&
		(ass1.lower <= ass2.higher) {
		return true
	}
	if (ass1.lower <= ass2.higher) &&
		(ass2.higher <= ass1.higher) {
		return true
	}
	if (ass2.lower <= ass1.higher) &&
		(ass1.higher <= ass2.higher) {
		return true
	}
	return false
...
func main() {
	...
	for _, line := range inputLines {
		...
		if isSolaped(assignments[0], assignments[1]) {
			totalCount++
		}
	}
	println("Total count for solaped assignments ", totalCount)
}
