...
func initIntMatrix(intMatrix *[][]int, startX int, startY int, endX int, endY int) {
	for i := 0; i < len(*intMatrix); i++ {
		for j := 0; j < len((*intMatrix)[0]); j++ {
			(*intMatrix)[i][j] = -1
		}
	}
	(*intMatrix)[startX][startY] = 0
	(*intMatrix)[endX][endY] = -2
}

func getSteps(intMatrix [][]int, stringMatrix [][]string, startX int, startY int, endX int, endY int) int {
...
func main() {
	...
	for i := 0; i < len(stringMatrix); i++ {
		for j := 0; j < len(stringMatrix[0]); j++ {
			if stringMatrix[i][j] == "a" {
				startX = i
				startY = j
				initIntMatrix(&intMatrix, startX, startY, endX, endY)
				res := getSteps(intMatrix, stringMatrix, startX, startY, endX, endY)
				if res > 0 && res < min {
					min = res
				}
			}
	...
	fmt.Println("El resultado es ", min)
